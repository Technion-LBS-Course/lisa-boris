...
YOLO11s is the **selected detector** because its measured detection and operational result files exist and it wins by the operational selection rule: the primary decision metric is its higher operational alert score, which already encodes its component hazard recall and false alert rate at the documented 10:1 weight, and on which YOLO11s also leads (higher hazard recall, lower false alert rate). Detection metrics and operational metrics are kept separate and are never compared against the sklearn baselines. YOLO11n remains the lightweight speed baseline / fallback. The dashboard loads these files as measured results automatically (see `src/results_loader.py` and `src/inference.py`).
...
YOLO11s remains the selected detector because it wins on the primary decision metric — the higher Operational Alert Score — which already encodes its component Hazard Recall and False Alert Rate at the documented 10:1 weight, and on which YOLO11s also leads (higher Hazard Recall, lower False Alert Rate); it is further supported by stronger object-detection metrics. The operational margin is modest — +0.0038 Operational Alert Score, reflecting 9 fewer missed hazards and 5 fewer false alerts — but the improvement is consistent. The detection-quality gain is clearer, especially mAP@0.5: 0.7668 for YOLO11s versus 0.7470 for YOLO11n (+1.98 pp), with consistent gains across mAP@0.5:0.95, Precision, Recall, and F1.
