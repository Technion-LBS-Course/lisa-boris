...
- **YOLO11s** — **planned primary detector**, subject to measured results.
...
7. **Do not choose YOLO11s merely because it is the planned primary detector.**
...
- **Interim runnable / selected detector:** the only currently measured, eligible detector. Today this is **YOLO11n**.
- **Final winner:** the detector selected **after a valid YOLO11n-versus-YOLO11s comparison** on equivalent conditions.

> **Important:** Because only YOLO11n currently has complete measured detector results, YOLO11n is the **only currently eligible measured detector / interim selected detector**. This is **not** proof that the required YOLO11n-versus-YOLO11s comparison is complete. The final comparison remains pending until measured YOLO11s results exist.

**Repository evidence (today):** `src/results_loader.select_operational_winner` implements this hierarchy via `_rank_candidates` (sort by `-hazard_recall`, then `false_alert_rate`, then `-operational_alert_score`, then `-detection_recall`, `-map50`, then `inference_ms`). With YOLO11s files absent, the test `test_pending_yolo11s_does_not_beat_measured_yolo11n` confirms the winner resolves to `YOLO11n`.
...
| YOLO11s training | Pending | `notebooks/PyroFinder_YOLO11s_Kaggle_Training.ipynb` present (untracked); training in progress | A completed Kaggle run producing the checkpoint |
| YOLO11s object-detection evaluation | Pending | No `results/baseline_yolo11s.json` (verified absent) | Produce measured detection JSON |
| YOLO11s operational evaluation | Pending | No `results/yolo11s_operational_metrics.json` (verified absent) | Run `scripts/evaluate_yolo_alert_metrics.py` after checkpoint exists |
| YOLO11s per-image prediction evidence | Pending | No `results/yolo11s_test_predictions.csv` (verified absent) | Generated by the same evaluation runner |
| Same-task detector comparison (YOLO11n vs YOLO11s) | Pending | Only YOLO11n measured; comparison code wired in `app.py` / `results_loader.py` | Measured YOLO11s detection + operational results under equivalent conditions |
| sklearn comparison table | Complete | `app.py` renders per-class metrics, distributions, and a model-comparison sub-tab | — |
| Object-detection comparison table | Partially complete | `app.py` "Object-detection comparison": YOLO11n measured, YOLO11s row = Training in progress | Fill YOLO11s row from measured file |
| Operational comparison table | Partially complete | `app.py` `_render_operational_alert_metrics`: sklearn + YOLO11n measured, YOLO11s pending | Fill YOLO11s operational row from measured file |
| Final detector winner selection | Partially complete | `select_operational_winner` selects YOLO11n as interim measured detector | Final winner requires measured YOLO11s and a valid two-detector comparison |
| Winner justification (written) | Partially complete | KPI hierarchy documented (§10); YOLO11n interim rationale available | Final KPI-based justification citing measured YOLO11n vs YOLO11s differences |
...
| Fine-tuned YOLO11s inference availability | Pending | `models/yolo11s_dfire_best.pt` absent (verified) | Add checkpoint after Kaggle run |
| Selected winner running in Streamlit | Partially complete | Interim selected detector (YOLO11n) runs end-to-end in the demo | Final winner run depends on completed comparison |
...
| README measured-result summary | Partially complete | README summarizes sklearn + YOLO11n detection + YOLO11n operational results | Add YOLO11s once measured |
| README selected-winner summary | Partially complete | README states YOLO11s training in progress; no final winner claimed | Final winner statement after the comparison completes |
| Final M3 submission evidence | Partially complete | Predictive path runnable on YOLO11n; sklearn + YOLO11n fully measured | Complete YOLO11n-vs-YOLO11s comparison and final winner justification |

---

## 16. Missing or pending evidence

The following items are **not yet satisfied** and are the critical path to a defensible M3 submission:

1. **Measured YOLO11s detection results** — `results/baseline_yolo11s.json` (absent).
2. **Measured YOLO11s operational results** — `results/yolo11s_operational_metrics.json` (absent).
3. **YOLO11s per-image predictions** — `results/yolo11s_test_predictions.csv` (absent).
4. **YOLO11s per-epoch curves** — `results/results_yolo11s.csv` (absent).
5. **YOLO11s fine-tuned checkpoint** — `models/yolo11s_dfire_best.pt` (absent).
6. **Completed YOLO11n-vs-YOLO11s comparison** under equivalent conditions (blocked on 1–5).
7. **Final KPI-based winner justification** referencing measured differences (blocked on 6).
8. **README updates** for YOLO11s measured results, final winner, and checkpoint placement (blocked on 1–7).

Generation command for the YOLO11s operational/location metrics once its checkpoint exists (evaluation only, no training):
...
- Pending YOLO11s rows must show **`Training in progress`** or **`Pending measured results`**.
...
**Repository evidence (today):** No synthetic YOLO11s result files exist (verified: no `*11s*` file in `results/`). `src/results_loader._NON_MEASURED_STATUS_KEYWORDS = ("synthetic", "placeholder", "pending", "training")` blocks any such file from selection; `tests/test_results_loader.py::test_synthetic_status_is_not_selectable` enforces it. The current honest state is **`Training in progress`** for YOLO11s everywhere.
...
> **Current gate status:** §18.1 and §18.2 are **not** fully satisfied because YOLO11s is not yet measured and the YOLO11n-vs-YOLO11s comparison is incomplete. §18.3 is satisfiable today on the interim measured detector (YOLO11n). §18.4 is partially satisfied (sklearn + YOLO11n complete; YOLO11s summary and final winner pending).
...
| `README.md` | Public-facing description, run instructions, measured-result summary (some YOLO11s sections pending). |
