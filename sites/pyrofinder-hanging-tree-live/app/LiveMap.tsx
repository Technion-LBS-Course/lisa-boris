...
export function LiveMap({ camera, cameraName, cameraShortName, anchors = [], incident, incidentLocation, incidentZone, fov, showWind, windObservation = PREPARED_WIND, interactive, selectedPoint, onPick, compact }: LiveMapProps) {
  ...
  useEffect(() => {
    ...
    let windLayer: import("leaflet").Layer | undefined;
    ...
    void import("leaflet").then(async (L) => {
      ...
      L.polygon(fov, { color: "#e4522f", weight: 1.5, fillColor: "#e4522f", fillOpacity: 0.12 }).addTo(map).bindTooltip("Camera field of view");
      ...
      if (incident) {
        const incidentIcon = L.divIcon({
          className: "pf-map-icon-shell",
          html: '<div class="pf-incident-icon">!</div><span>DETECTED INCIDENT</span>',
          iconSize: [132, 34], iconAnchor: [15, 17],
        });
        L.marker(incidentLocation, { icon: incidentIcon }).addTo(map).bindPopup(`Detected incident — ${incidentZone}`);
      }
    ...
    return () => {
      disposed = true;
      if (map && windLayer && map.hasLayer(windLayer)) map.removeLayer(windLayer);
      map?.off();
      map?.remove();
      windLayer = undefined;
      map = undefined;
