...
import { confirmedKinds as getConfirmedKinds, detectionGeometry, detectionKind, displayedFrameNumber, nextAlertFocus, qualifyingDetections, selectAlertFocus, timelineMarkers, zoneForDetection } from "./detection-logic";
import { detailedWeatherMessage, detectionAlertMessage, generalWindWatchMessage, observationTime } from "./incident-guidance";
...
type EventRecord = { id: string; camera: CameraId; timestamp: string; kind: "smoke" | "fire"; status: "confirmed" | "false_alarm"; confidence: number; zone: string; frame: number; detections?: Detection[] };
...
function CameraFrame({ config, frame, detections, anchors, zones, onPick, pending, pendingMode = "box", pendingReference }: {
  ...
  return (
    <div className={`camera-frame ${onPick ? "pickable" : ""}`} onClick={click}>
      ...
      {detections?.map((d, i) => {
        const geometry = detectionGeometry(d);
        return geometry && <div key={`${d.kind}-${i}`} data-detection-kind={d.kind} className={`detection-box ${d.kind}`} style={{ left: pct(geometry.left), top: pct(geometry.top), width: pct(geometry.width), height: pct(geometry.height) }}><span>{d.kind.toUpperCase()} {Math.round(d.confidence * 100)}%</span></div>;
      })}
...
function CameraWorkspace({ cameraConfig, onSelectCamera }: { cameraConfig: CameraConfig; onSelectCamera: (cameraId: CameraId) => void }) {
  ...
  const currentDetections = useMemo(
    () => qualifyingDetections(detections[frame] || [], { smoke: smokeThreshold / 100, fire: fireThreshold / 100 }),
    [detections, frame, smokeThreshold, fireThreshold],
  );
  const revealedMarkers = useMemo(
    () => timelineMarkers(detections, highestReachedFrame, { smoke: smokeThreshold / 100, fire: fireThreshold / 100 }),
    [detections, highestReachedFrame, smokeThreshold, fireThreshold],
  );
  const rawKind = detectionKind(currentDetections);
  const confirmedKinds = useMemo(() => {
    return getConfirmedKinds(frame, confirmationFrames, detections, { smoke: smokeThreshold / 100, fire: fireThreshold / 100 });
  ...
  const alertZone = useMemo(() => activeAlert ? zoneForDetection(activeAlert, zones) : null, [activeAlert, zones]);
  const alertAnchor = useMemo(() => activeAlert ? detectionGeometry(activeAlert)?.anchor : undefined, [activeAlert]);
  const alertLocation = useMemo(
    () => alertZone && alertAnchor ? projectImagePointToMap(alertAnchor, anchors) : null,
    [alertZone, alertAnchor, anchors],
  );
  const alertZoneLabel = alertZone?.name ?? "Unmapped camera area";
  ...
  useEffect(() => {
    if (frame > highestReachedFrame) queueMicrotask(() => setHighestReachedFrame(frame));
  }, [frame, highestReachedFrame]);
  ...
  }, [playing, frame, speed, totalFrames]);
  ...
  useEffect(() => {
    ...
    if (confirmedFocus) {
      ...
      queueMicrotask(() => {
        ...
        setPlaying(false);
        setChat(c => [...c, { agent: "Response", role: "assistant", text: detectionAlertMessage(focus, zones, cameraConfig.name, currentDownwindDirection) }]);
      });
    }
  }, [currentDetections, activeAlert, confirmedFocus, cameraConfig, currentDownwindDirection, zones]);

  useEffect(() => {
    if (!activeAlert) return;
    const nextFocus = nextAlertFocus(activeAlert, confirmedFocus);
    if (nextFocus === activeAlert) return;
    if (nextFocus) {
      const fireEscalation = activeAlert.kind === "smoke" && nextFocus.kind === "fire";
      queueMicrotask(() => {
        setActiveAlert(nextFocus);
        if (fireEscalation) {
          setPlaying(false);
          setChat(current => [...current, { agent: "Response", role: "assistant", text: detectionAlertMessage(nextFocus, zones, cameraConfig.name, currentDownwindDirection) }]);
        }
      });
      return;
    }
  ...
  }, [activeAlert, confirmedFocus, cameraConfig.name, currentDownwindDirection, zones]);
  ...
  const resolveAlert = (status: "confirmed" | "false_alarm") => {
    if (!activeAlert) return;
    const record: EventRecord = { id: crypto.randomUUID(), camera: cameraConfig.id, timestamp: new Date().toISOString(), kind: activeAlert.kind, status, confidence: activeAlert.confidence, zone: alertZoneLabel, frame, detections: currentDetections };
  ...
  const deterministicReply = (value: string) => {
    ...
    if (!activeAlert) return "No active detection. Monitoring continues.";
    if (lower.includes("call") || lower.includes("contact")) return "For an emergency, call 911. I can draft a concise report using this camera’s approved operational context.";
    if (lower.includes("where") || lower.includes("location")) return `Detected zone: ${alertZoneLabel} in the ${cameraConfig.name} view.`;
    if (lower.includes("draft") || lower.includes("message")) return `Draft: “PyroFinder ${cameraConfig.name} observed ${activeAlert.kind} in ${alertZoneLabel}. Please verify conditions and keep access clear.”`;
    return "The event meets the configured detection rule. I can suggest relevant contacts or draft a notification.";
  };
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          incident: activeAlert ? { class: activeAlert.kind, raw_detections: currentDetections, temporal_status: "confirmed", camera: cameraConfig.name, zone: alertZoneLabel, approximate_location: alertLocation, location_status: alertLocation ? "camera-projected approximation" : "unmapped", frame_index: frame, displayed_frame: displayedFrameNumber(frame), wind_direction: currentWindLabel, wind_speed_m_s: weatherObservation.speedMs, prototype_fire_weather_risk: weatherObservation.riskLevel, operator_status: "awaiting resolution", detector: cameraConfig.detector } : { status: currentDetections.length ? "raw detection; not temporally confirmed" : "no detection", raw_detections: currentDetections, detector: cameraConfig.detector },
        }),
      });
  ...
  const finishReferencePoint = () => {
    ...
    setZoneMessages(m => [...m, { role: "assistant", text: "Reference point saved. Incidents in this zone will use it for map reporting, and the mapped field of view has been updated." }]);
  };
