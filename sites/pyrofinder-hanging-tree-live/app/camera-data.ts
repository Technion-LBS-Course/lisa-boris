...
import type { Detection } from "./detection-logic";

export type CameraId = "hanging-tree-1" | "thunder-valley-west";
export type Priority = "high" | "medium" | "low";
export type { Detection } from "./detection-logic";
...
const APPROVED_CHECKPOINT_SHA256 = "6AA0C7DCD60E3572F85F02EDC05293266822F9394944479337BEBB8D178B6903";
...
const detectorFromResults = (results: ResultFile): DetectorProvenance => {
  const classes = results.fingerprint.classes ?? {};
  if (results.fingerprint.model !== "YOLO11s"
    || results.fingerprint.output_kind !== "verified-ultralytics-inference"
    || results.fingerprint.checkpoint_sha256 !== APPROVED_CHECKPOINT_SHA256
    || Object.keys(classes).length !== 2
    || classes["0"] !== "smoke"
    || classes["1"] !== "fire"
    || !Array.isArray(results.frames)
    || results.fingerprint.n_frames !== results.frames.length
    || !Number.isFinite(results.fingerprint.collection_confidence)
    || results.fingerprint.collection_confidence < 0
    || results.fingerprint.collection_confidence > 1
    || !Number.isInteger(results.fingerprint.imgsz) || results.fingerprint.imgsz <= 0
    || results.frames.some((frame, index) =>
      frame.name !== `frame_${String(index).padStart(2, "0")}.jpg`
  ...
  results.frames
    .map((frame, index) => [
      ...
      frame.detections.map(detection => {
        if ((detection.class !== "smoke" && detection.class !== "fire")
          || !Number.isFinite(detection.confidence)
          || detection.confidence < results.fingerprint.collection_confidence
          || detection.confidence > 1
          || detection.bbox_norm.length !== 4
          || detection.bbox_norm.some(value => !Number.isFinite(value) || value < 0 || value > 1)
          || detection.bbox_norm[2] <= 0 || detection.bbox_norm[3] <= 0) {
          throw new Error(`Malformed YOLO detection in result frame ${index}`);
        }
        return {
          kind: detection.class,
