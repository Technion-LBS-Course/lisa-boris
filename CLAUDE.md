...
src/tracking.py                     — multi-frame confirmation (is_confirmed_detection: strict, unbroken last-N-frames) + one-miss-tolerant confirmation (is_confirmed_with_tolerance: current frame must be positive AND at least N of the last N+1 frames positive — used by the M4 demo sequence player) + apparent direction estimation
src/mapping.py                      — mapping modes, polygon helpers, approximate location formatting; per-zone image reference point helpers (zone_reference_point_norm, validate_zone_reference_point, generate_zone_map_estimates): each image zone may carry one operator-set map-reporting point (zone_ref_point_px / zone_ref_point_norm) — the polygon is image-space zone membership only; only the zone reference point is projected to the map (never polygon vertices or centroid; no polygon stretching on the map)
...
src/inference.py                    — lazy YOLO11n/YOLO11s detector loading (ultralytics imported inside functions only) + single-image detection; fine-tuned D-Fire checkpoints only, never pretrained weights; validates fire/smoke-only classes. top_hazard_detection() picks the fire-first event focus within one frame; select_confirmed_event_detection() extends the same fire-over-smoke priority across an N-frame confirmation window (current frame's fire wins; else the most recent fire in an earlier window frame; else the current frame's smoke) — used when a confirmed M4 sequence incident is built. Both are pure and testable without a loaded model.
...
src/incident_agent.py               — Incident Assistant: build_incident_context() (matched image zone, approximate map point, apparent image-plane direction, optional downwind), recommend_actions(), draft messages (owner/neighbor/farm worker/fire-dept summary), create_incident_alert(); optional Groq wording polish. Map-point priority: a detection inside a zone uses the zone's operator-set reference point (map_point_source="zone_reference_point"); a matched zone without a reference point gets NO invented map point (the missing point is reported); only when no zone matches is the detection anchor projected (map_point_source="detection_anchor"), else image-quadrant/camera-frame text. Drafts and recommends only — never contacts anyone or dispatches. Pure, no ML.
...
src/dashboards/central_control.py   — Central Control dashboard renderer, six tabs: Camera Metadata, Map Reference Points, Image Zones, Export & Generate, Incident Assistant, Risk Advisory. Includes an "Import saved configuration (JSON)" uploader (inverse of export). Image Zones default to AI-assisted: "Structure from text" (src/zone_agent.parse_zone_description → operational zone records with object_to_find + low/medium/high priority; Groq-or-local; accept as pending or draw each polygon) and "Detect on image" (src/llm.detect_zone_boxes → approximate vision ROI boxes); a "Switch to manual drawing" button gives the click-to-draw flow. Once a rough ROI box exists (AI ROI, manual box, or the bounding box of clicked points), a "Selected ROI box" selector + "Refine selected box with segmentation" button runs src/segmentation_assist (local OpenCV GrabCut, on click only — not YOLO/Groq) to produce a cleaner polygon the operator accepts (loaded into the editor for save) or falls back to the box-as-polygon; wired into both the AI and manual panels. The zone editor's click action toggles between adding polygon vertices and setting the per-zone "zone reference point" (drawn as a diamond marker; saved as zone_ref_point_px/zone_ref_point_norm; loaded/cleared with Load into editor / Cancel edit; validation warnings — never blockers — when missing, out of bounds, outside the polygon, or above the estimated skyline, which stays a visual setup aid only). Export & Generate's "Generate Map Estimate" projects each enabled zone's zone reference point only (src/mapping.generate_zone_map_estimates; zones without one are skipped with a count; no vertices_latlon / polygon projection), and the config JSON export/import round-trips the reference point fields. Incident Assistant uses src/incident_agent; Risk Advisory uses src/weather. The shared frame uploader also offers a "Demo: image sequence" loader (folder path — default TESTING/sequance — or multi-file upload) that resizes all frames to one common size and steps through them (Prev/Next/slider), driving cc_uploaded_image so Image Zones and the Incident Assistant run per selected frame — reuses run_detection, no YOLO changes. No module-level YOLO/torch imports.

Incident Assistant sequence-view playback and N-frame confirmation (M4 only; Central Control's own Incident Assistant call keeps sequence_view=False and is unaffected): "Play"/"Pause"/"Stop" buttons + a speed slider (_playback_controls, cc_seq_playing/cc_seq_frame_delay_ms) autoplay the loaded demo sequence purely via session state + st.rerun() — a single bounded time.sleep() per rerun (_advance_playback_if_needed), never a thread/async scheduler; playback stops automatically at the last frame. Because cc_seq_idx is bound to the sequence slider's widget key inside _sequence_panel(), it can never be written after that widget renders in the same run (Streamlit raises StreamlitAPIException) — so Stop and the autoplay advance queue their target index in cc_seq_pending_seek, applied by _apply_pending_seq_seek() at the top of the next run, before _sequence_panel() creates the slider (mirrors how the existing Prev/Next buttons already mutate cc_seq_idx before that same widget). A configurable "Confirmation frames (N)" number input (cc_incident_confirm_n, default 3) drives _process_current_sequence_frame(), which uses _ensure_window_detections() to reuse cc_seq_det for already-cached frames and only computes the small bounded gap (at most N+1 frames) needed for the trailing confirmation window, then calls src.tracking.is_confirmed_with_tolerance() (one-miss tolerance: current frame positive AND >= N of the last N+1 positive). cc_incident_ctx is built at most once per confirmed event (_maybe_build_confirmed_incident, guarded by cc_incident_confirmed_idx so autoplay never spam-creates incidents on later positive frames); _reset_incident_state() (Clear incident / Confirm alert / Mark false alarm) clears that guard so a later window can confirm again. _build_confirmed_incident() anchors the incident on src.inference.select_confirmed_event_detection()'s fire-priority pick across the window, not just the current frame. Before confirmation, the right column shows a compact status via _render_pending_incident_status() ("No fire/smoke detection in this frame" or "Detection observed — waiting for N-frame confirmation (x of last y frame(s) positive)") instead of the chat; the left-column incident map shows a distinct "unconfirmed" preview marker (_preview_map_point(), _render_incident_map(..., preview_point=...)) for the current frame's detection — matched-zone reference point first, else the existing detection-anchor projection (more lenient than the confirmed incident's stricter zone-matched-without-ref-point rule, since it is only a live indicator).
...
src/dashboards/m4_dashboard.py      — M4 dashboard renderer; reuses five Central Control tabs (Camera Metadata, Map Reference Points, Image Zones, Incident Assistant, Risk Advisory) via src/dashboards/central_control, sharing its cc_* session state (only one mode renders per run, so widget keys never collide). The Map Reference Points section renders _frame_uploader(with_sequence=False) above cc._tab_map_reference_points() so the stable reference frame is available for pixel picking — no reference-point logic is duplicated. M4 tailors the Incident Assistant with flags — _tab_incident_assistant(show_intro=False, allow_manual_point=False, sequence_view=True, show_drafts=False): hides the workflow blurb, the manual hazard-point fallback, and the Draft-messages section, and adds an in-tab demo-sequence loader + Play/Pause/Stop/speed playback controls + slider that auto-runs YOLO per selected frame and overlays boxes (cached in cc_seq_det). When a sequence is loaded the manual "Run YOLO11s" button is hidden and the incident is built only once N-frame confirmation is satisfied (see central_control.py's Incident Assistant sequence-view entry above for the full playback/confirmation pipeline). In M4 the sequence panel runs with drive_shared_frame=False: the incident slider drives only the incident's own per-frame detection, while Image Zones / Camera Metadata keep the stable reference frame (first sequence frame). In M4 the shared camera-frame uploader (_frame_uploader(with_sequence=False)) and import-config panel live inside the Camera Metadata section; that uploaded reference frame is used by Image Zones, while the Incident Assistant's demo sequence is decoupled (drive_shared_frame=False everywhere, so loading a sequence never overwrites the reference frame). M4 selects its five sections with a keyed st.segmented_control (not st.tabs) so the active section is stored in session state and survives every rerun (st.tabs has no key and resets to the first tab on reruns — uploads, sliders, etc.); only the active section renders. In M4 sequence mode the incident map renders taller (_render_incident_map(ctx, map_height=720)) to span the right-side frame + conversation column; Central Control keeps the compact default. No module-level ML imports.
...
tests/test_inference.py             — unit tests for src/inference.py (checkpoint paths, class validation, missing-checkpoint guard, top_hazard_detection, select_confirmed_event_detection fire-priority-across-window) — no real weights, no ultralytics import
tests/test_tracking.py              — unit tests for src/tracking.is_confirmed_with_tolerance (one-miss tolerance, trailing-window-only, current-frame-must-be-positive, required_frames validation) — pure
